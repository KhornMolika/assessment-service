...
function buildCsv(rows: ResultsRow[]) {
  ...
  const records = rows.map((row) => [
    ...
    row.totalScore,
    row.maxScore,
    row.answerSheetStatus === "IN_PROGRESS" ? "In Progress" : row.percentage == null ? "Pending review" : `${row.percentage}%`,
    row.answerSheetStatus === "IN_PROGRESS" ? "In Progress" : row.grade ?? "Pending",
